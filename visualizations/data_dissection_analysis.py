...
for filename in all_structured_files:
    ...
    for segment_num in f['segments'].keys():
        for timepoint_num in range(np.array(f['segments'][segment_num]['left']).shape[0] - n_tpts_for_targs + 1):
...
img_l = cv2.cvtColor(l_frames[0], cv2.COLOR_BGR2GRAY)
img_r = cv2.cvtColor(r_frames[0], cv2.COLOR_BGR2GRAY)
...
plt.imshow(img_r)
for i in range(4):
    stereo = cv2.StereoBM(preset=1,ndisparities=i*16)
    disp = stereo.compute(img_l,img_r)
    f.add_subplot(3,2,3+i)
    plt.imshow(disp)
plt.show()
...
'''
            ...
            for tpt in range(stride * nsteps - 1, -1, -stride)[::-1]:
                moment_m_targs.append(f['segments'][segment_num]['motor'][timepoint_num+tpt])
                moment_s_targs.append(f['segments'][segment_num]['steer'][timepoint_num+tpt])

            m_targs.append(f['segments'][segment_num]['motor'][timepoint_num+(stride * nsteps - 1)])
            s_targs.append(f['segments'][segment_num]['steer'][timepoint_num+(stride * nsteps - 1)])

            m_targs_avg.append(np.mean(moment_m_targs))
            s_targs_avg.append(np.mean(moment_s_targs))

            m_targs_std.append(np.std(moment_m_targs))
            s_targs_std.append(np.std(moment_s_targs))

np.save('structured_targs_stats.npy', [m_targs,s_targs,m_targs_avg,s_targs_avg,m_targs_std,s_targs_std])

structured_stats = np.load('structured_targs_stats.npy')
unstructured_stats = np.load('unstructured_targs_stats.npy')

nbins = 21

stats = structured_stats.copy()
f = plt.figure()
f.add_subplot(3,2,1)
plt.hist(stats[0], nbins) # motor value 1s after start of data moment
f.add_subplot(3,2,2)
plt.hist(stats[1], nbins) # steer value 1s after start of data moment
f.add_subplot(3,2,3)
plt.hist(stats[2], nbins) # average motor value across 1s after start of data moment
f.add_subplot(3,2,4)
plt.hist(stats[3], nbins) # average steer value across 1s after start of data moment
f.add_subplot(3,2,5)
plt.hist(stats[4], nbins) # stddev of motor value across 1s after start of data moment
f.add_subplot(3,2,6)
plt.hist(stats[5], nbins) # stddev of steer value across 1s after start of data moment
plt.show()
'''
