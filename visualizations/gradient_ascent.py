...
##
## BALA TO DO: REMOVE DROPOUT + READD JITTER
##
...
for layer_to_vis in range(0,16):  # 16 total functions in our squeezenet
    ...
    dims_to_vis = [2]# BALA changed this: np.arange(output.size()[1])  # channel
    learning_rate = 10000
    for dim_to_vis in dims_to_vis:
        ...
        for asc_iter in range(1000):
            ...
            # performing jitter is convenient b/c it also resets Variable so
            # it's not volatile for back-prop
            #camera_data = Variable(
            #    torch.from_numpy(jittered_camera_data).cuda(),
            #    requires_grad=True)

            camera_data = Variable(torch.from_numpy(camera_data.data.cpu().numpy()).cuda(), requires_grad=True)
            ...
            if len(output.size()) > 2:
                ...
                # this is the dim that gets visualized, all others' gradients
                # set to 0
                
                # BALA changed this
                #grad_temp1 = torch.ones(batch_size, 1, height, width).cuda()
                
                grad_temp1 = torch.zeros(batch_size, 1, height, width).cuda()
                grad_temp1[:,:,0,0] = 1*learning_rate
                
                grad_temp2 = torch.zeros(
                    batch_size, dim_size - dim_to_vis - 1, height, width).cuda()
            else:
                grad_temp0 = torch.zeros(batch_size, dim_to_vis).cuda()

                # BALA changed this
                grad_temp1 = learning_rate*torch.ones(batch_size, 1).cuda()                
            ...
            if dim_to_vis == 0:
                ...
                gradients = torch.cat(
                    (grad_temp0, grad_temp1, grad_temp2), dim=1)
            ...
            if (asc_iter + 1) % 100 == 0:
                print(g_abs_mean)
                print(
                    'layer_{}_channel_{}_iter_{}.png'.format(
                        layer_to_vis,
                        dim_to_vis,
                        asc_iter + 1))
