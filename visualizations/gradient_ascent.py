...
layer_to_vis = 6 #len(all_layer_funcs)-1 ## SHOULD BE A CONTROLLABLE PARAMETER
...
for dim_to_vis in dims_to_vis:
    camera_data = Variable(255*torch.randn(1,12,94,168).cuda(), requires_grad=True) ## RESETS FOR EACH DIM SO NOT VOLATILE
    ...
    metadata = torch.cat((zero_matrix, metadata), 1)
    metadata = torch.cat((zero_matrix, metadata), 1)
    metadata = torch.cat((torch.FloatTensor(1, 122, 23, 41).zero_().cuda(), metadata), 1)  # Pad empty tensor            
    metadata = Variable(metadata.cuda())

    for asc_iter in range(1000):
        jitter = 32
        ox, oy = np.random.randint(-jitter, jitter+1, 2)
        jittered_camera_data = np.roll(np.roll(camera_data.data.cpu().numpy(), ox, -1), oy, -2)
        camera_data = Variable(torch.from_numpy(jittered_camera_data).cuda(), requires_grad=True) # performing jitter is convenient b/c it also resets Variable so it's not volatile for back-prop
        ...
        camera_data.data.add_(torch.from_numpy(g*learning_rate/g_abs_mean).cuda())

        if (asc_iter+1)%100 == 0:
            print('layer_{}_channel_{}_iter_{}.png'.format(layer_to_vis, dim_to_vis, asc_iter+1))
            ...
            scipy.misc.imsave('rf_imgs/layer_{}_channel_{}_iter_{}.png'.format(layer_to_vis, dim_to_vis, asc_iter+1), img)
            
        unjittered_camera_data = np.roll(np.roll(camera_data.data.cpu().numpy(), -1*ox, -1), -1*oy, -2)
        camera_data = Variable(torch.from_numpy(unjittered_camera_data).cuda(), requires_grad=True)
