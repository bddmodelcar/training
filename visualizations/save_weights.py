...
def convert_conv1_weights_to_img(weights):
    ...
    for out_dim in range(temp_weights.shape[0]):
        xshape = temp_weights.shape[2]
        yshape = temp_weights.shape[3]
        ...
        for t in range(n_frames):
            img = np.zeros((x_shape, 2*y_shape, 3))
            
            for camera in ('left', 'right'):
                for c in range(3):
                    if camera == 'left':
                        img[t * xshape:(t + 1) * xshape, 0:yshape, c] = temp_weights[out_dim, ctr, :, :]
                    elif camera == 'right':
                        img[t * xshape:(t + 1) * xshape, yshape:,  c] = temp_weights[out_dim, ctr, :, :]
                    ctr += 1
...
model_path = '/home/bala/epoch00.weights'
