...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(metadata_file['state'][0]); print(metadata_file['state'][1]);print(metadata_raw['direct'][0])

        if self.separate_frames:
            metadata = torch.FloatTensor(self.nframes, 64, 23, 41)
        else:
            #metadata = torch.FloatTensor(self.nframes * 64, 23, 41)
            metadata = torch.FloatTensor(6, 11, 20)

        metadata_count = 5
        for cur_label in ['racing', 'caffe', 'follow', 'direct', 'play',
                          'furtive']:
            if cur_label == 'caffe':
                if metadata_file['state'][0]:
                    metadata[metadata_count, :, :] = 1.
                else:
                    metadata[metadata_count, :, :] = 0.
            else:
                if cur_label in metadata_raw and metadata_raw[cur_label]:
                    metadata[metadata_count, :, :] = 1.
                else:
                    metadata[metadata_count, :, :] = 0.
            metadata_count -= 1
